...
;(async () => {
  ...
  if (answers.version === 1) {
    ...
    if (answers.chain === 'Mainnet') {
    ...
    } else if (answers.chain === 'Arbitrum') {
      ...
      optimismConnectorsV2[answers.name] = answers.address
      connectorsV2Template += JSON.stringify(optimismConnectorsV2, null, 4)

      // save the file
      fs.writeFileSync(optimismAddressV2Path, connectorsV2Template)
      console.log(`🚀 ${optimismAddressV2Path} [updated]`)
    }
  }
